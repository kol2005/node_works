...
// 매개변수로 config 값을 전달하기 위해서 module.exports를 이전과 다른 방법으로 선언
module.exports = function(app,config){
    ...
    */
    ...
    // 이하의 router method들을 실행하기 전에 먼저 실행하여 
    // 변수, 세팅등을 설정한 후 req로 요청한 다음번(next) router로 전달하라
    ...
    router.get("/viewStation",function(req,res){
        ...
        // 전달되어 오면 그 페이지 번호에 pageLimit값을 곱하여
        // 몇번째 데이터부터 읽어 올 것인지 정하는
        // 값을 계산
        // offset == skip
        ...
        .exec(function(err,count){
            ...
            // 위에서 계산한 pageLimit,offset 값을 활용하여 필요한 데이터만 추출한다
            gjStation.find({})
            .limit(pageLimit)
            .skip(offset)
            .exec(function(err,stations){
