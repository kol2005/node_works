...
moment.tz.setDefault("Asia/Seoult");

/*
.once("value")
- firebase DB에서 데이터를 조회할떄 사용하는 이벤트 메서드
 db.ref("bbs") : firebase에 데이터를 요청하는 호출메서드
 데이터 요청이 허락되면 value 라는 이름으로 
 응답을 수행하는데 그때 응답을 수신하는 메서드
...
router.post("/input", (req, res) => {
  ...
  // db접속을 CRUD 매번마다 접속하여 수행하라 라는 문서상 규칙이 있다
  var db = firebase.database();
  ...
  /*
  form에서 전송되어온 seq 값이 없으면 새로운 seq를 생성해서
  insert 상태로 만들고 날짜와 시각을 생성하여 data(req.body)에 추가
  js 에서는 "", undefined : false
  */
  if (!seq) {
