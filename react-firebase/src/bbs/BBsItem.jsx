...
2. 이벤트 핸들러 등록 onClick={handlerEvent} 핸들러 이벤트를 등록할때는 함수를 객체 타입으로 바꾸어서 등록을 한다

= 함수 이름에서 괄호를 반드시 제거하고 등록 만약 이벤트 핸드럴를 함수 타입
= 함수() 으로 등록을 하게되면 각 컴포넌트가 랜더링 되는 동안 이벤트 핸들러도 등록하는 것이 아니라 함수를 호출하여 실행해 버린다
그런데 이벤트 핸들러에게 어떤 값을 전달하여 그 값을 핸들러 코드내에서 사용하고자 할때는?
가상의(익명의) 이벤트 핸들러를 등록하고
(e)=>{}
이 익명 이벤트 핸들러에서 함수를 호출하도록 코드를 작성해 줘야한다
(e)=>{ 함수(e,매개변수) }
이렇게 등록을 하면 렌더링하는 동안에는
(e)=>{} 의 익명 이벤트만 바라보고 이벤트 핸들러로 등록ㄹ하며
이때 내부에 작성된 코드는 모두 무시된다
비로소 모든 렌더링이 끝나고 이벤트 실행을 하게되면 이벤트 핸들러가 자신이 포함하고 있는
