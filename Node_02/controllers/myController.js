...
router.get("/",function(req,res){
    ...
    // write() 함수를 사용해서 web브라우저에 문자열 형태로 데이터를 표시하도록 하기
    // write() 문자열들을 전송하고 끝에 반드시 end()를 전송해 줘야 한다
...
// 첫번째 파라메터(req)는 web에서 전송되는 request 정보가 담길 변수
// 두번째 파라메터(res)는 서버에서 web에게 응답할때 데이터를 담거나 여러 정보를 담아서 보낼 객체
...
// 서버에 request를 요청할때 query String으로 데이터를 보내면
