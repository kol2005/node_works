...
class BucketItemEdit extends Component {
  ...
  */
  ...
  onChange = ev => {
    this.setState({ bucket_title: ev.target.value });
  };
    ...
    return (
      <React.Fragment>
        ...
        <td onClick={this.changeEdit}>
          ...
          그래야만 onChange 이벤트에서 키보드로 입력한 내용을 정상적으로 
          input box에 보여주도록 할수 있기 때문에

          그러나, state.bucket_title은 초기값이 "" 인 비어있는 값이다
          그럼 props로 전달받은 b_title을 state.bucket_title에 주입하여
          보여주고 수정할수 있도록 해야 하는데
          props로 받은 값을 state형 변수에 주입하기 위해서는 
          LifeCycle method 중에서 componentDidUpdate() 에서 처리를 해줘야 한다 */}
          <input
